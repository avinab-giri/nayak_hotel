...
<body class="g-sidenav-show  bg-gray-100">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg scrollBar">
        ...
        <div class="container">
            ...
            <?php
            echo backNavbarUi('', 'Guest Birthday');
            ?>
    ...
    <script>
        ...
        $('#SearchByDateFrom').on('change', function(e) {
            var date = $(this).val();
            const toDate = $('#SearchByDateTo').val();
            loadGuesBirthReport(date,toDate);
        });

        $('#SearchByDateTo').on('change', function(e) {
            var date = $(this).val();
            const fromDate = $('#SearchByDateFrom').val();
            loadGuesBirthReport(fromDate,date);
        });

        function loadGuesBirthReport(from = '',to = '', name = '') {
            var data = `request_type=loadGuesBirthReport&from=${from}&to=${to}`;
            ...
            ajax_request(data).done(function(request) {
                ...
                if (response.length > 0) {
                    $.each(response, function(key, val) {
                        ...
                        var eventDateFormat = moment(`${birthday}-2024`).format('DD-MMM');
