...
<body class="g-sidenav-show  bg-gray-100">
    ...
    <script>
        ...
        $('#SearchByDateFrom').on('change', function(e) {
            ...
            loadGuestDataReport(date,toDate);
        });

        $('#SearchByDateTo').on('change', function(e) {
            var date = $(this).val();
            const fromDate = $('#SearchByDateFrom').val();
            loadGuestDataReport(fromDate,date);
        });

        function loadGuestDataReport(from = '',to = '', name = '') {
            var data = `request_type=loadGuestDataReport&from=${from}&to=${to}`;
